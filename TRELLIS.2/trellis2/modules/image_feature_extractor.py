    ...
    def __init__(self, model_name: str, image_size=512):
        ...
        hf_to_local_map = {
            'facebook/dinov3-vitl16-pretrain-lvd1689m': 
                os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', '..', '..', '..', 'models', 'trellis2', 'facebook', 'dinov3-vitl16-pretrain-lvd1689m'),
        }
        
        # Check if model_name is a HF repo ID and map to local path
        local_model_path = model_name
        for hf_repo_id, local_path in hf_to_local_map.items():
            if model_name == hf_repo_id:
                if os.path.exists(local_path):
                    print(f"[TRELLIS.2] ✓ Mapping DINOv3 HF repo '{hf_repo_id}' to local path: {local_path}")
                    local_model_path = local_path
                else:
                    print(f"[TRELLIS.2] ✗ Local path not found for DINOv3: {local_path}")
                break
        
        # Load model from local path or HF repo
        self.model = DINOv3ViTModel.from_pretrained(local_model_path)
        self.model.eval()
        self.transform = transforms.Compose([
            transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
        ])
